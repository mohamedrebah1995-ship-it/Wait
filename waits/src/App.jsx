...
// In dev, Vite proxies /auth and /waits to localhost:3001.
// In prod, the same Express server handles everything.
const DEV      = import.meta.env.DEV;
const API_BASE = DEV ? "http://localhost:3001" : "";
const wsProto  = () => (location.protocol === "https:" ? "wss" : "ws");
const WS_BASE  = DEV ? "ws://localhost:3001" : `${wsProto()}://${location.host}`;
...
function getCommunityWait(restId, now, patterns) {
  ...
  // Best matching bucket: most specific first
  ...
  async function submit(e){
    ...
    try{
      const res=await fetch(`${API_BASE}/auth/${mode}`,{
        ...
        body:JSON.stringify({username:username.trim(),password,color,initial:username.trim()[0].toUpperCase()}),
      });
      const data=await res.json();
      if(!res.ok){setError(data.error||"Something went wrong");return;}
      store.set("delivr_token",data.token);store.set("delivr_user",data.user);
      onLogin(data.user,data.token);
    }catch(err){setError("Cannot reach server — is it running?");
    ...
    <div style={{padding:"20px 16px 100px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {sorted.map(r=>{
          ...
          // Big display number: personal > community > database
          ...
          return(
            <div key={r.id} style={{background:isActive?"#150900":"#0d0d0d",borderRadius:12,border:"1px solid "+(isActive?"#ff6600":riskColor+"33"),padding:"14px 16px"}}>
              ...
              {/* Data boxes: personal | community | database */}
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                {community&&<span style={{fontSize:9,color:"#00aaff44"}}>·{community.count} community</span>}
...
function ChatScreen({ user, token, onLogout }) {
  ...
  const connect=useCallback(()=>{
    ...
    ws.onmessage=e=>{
      try{
        ...
        if(d.type==="message") setMessages(p=>[...p,d.message]);
        if(d.type==="online")  setOnline(d.count);
        // patterns are handled at root level via the root WS
      }catch(err){}
    };
    ws.onclose=e=>{
  ...
  useEffect(()=>{connect();return ()=>{clearTimeout(reconnTimerRef.current);wsRef.current?.close();};},[connect]);
  useEffect(()=>{bottomRef.current?.scrollIntoView({behavior:"smooth"});},[messages.length]);

  function send(){
    if(!input.trim()||wsRef.current?.readyState!==WebSocket.OPEN)return;
    wsRef.current.send(JSON.stringify({type:"chat",text:input.trim()}));
  ...
  const statusDot={connecting:"#ffd600",connected:"#00e87a",reconnecting:"#ff3232"}[status];
      ...
      <div style={{padding:"10px 16px",borderBottom:"1px solid #111",background:"#080808",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:5,marginTop:2}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:statusDot,boxShadow:"0 0 6px "+statusDot,animation:status==="connected"?"criticalPulse 2.5s ease-in-out infinite":"none"}}/>
            <span style={{fontSize:9,color:"#444",letterSpacing:1}}>{status==="connected"?online+" ONLINE":status.toUpperCase()+"..."}</span>
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"10px 14px 6px",display:"flex",flexDirection:"column",gap:2}}>
        {messages.length===0&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,paddingBottom:40}}>
            {status!=="connected"
              ?<div style={{...B,fontSize:14,color:statusDot,letterSpacing:2,animation:"criticalPulse 1.2s ease-in-out infinite"}}>{status.toUpperCase()}...</div>
              :<><div style={{fontSize:38,opacity:0.2}}>💬</div><div style={{...B,fontSize:18,color:"#222",letterSpacing:2}}>NO MESSAGES YET</div></>
            }
          </div>
        )}
        {messages.map((m,i)=>{
      ...
      <div style={{padding:"10px 12px 14px",borderTop:"1px solid #111",background:"#080808",flexShrink:0,display:"flex",gap:10,alignItems:"center"}}>
        <div style={{flex:1,background:"#111",border:"1px solid #222",borderRadius:24,padding:"11px 18px",display:"flex",alignItems:"center"}}>
          <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey}
            placeholder={status==="connected"?"Say something to the group...":"Waiting for connection..."}
            disabled={status!=="connected"} maxLength={500}
            style={{flex:1,background:"none",border:"none",color:"#f0f0f0",fontSize:14,...M,outline:"none",opacity:status==="connected"?1:0.4}}
            onFocus={e=>{e.target.parentElement.style.borderColor="#ff6600";}}
            onBlur={e=>{e.target.parentElement.style.borderColor="#222";}}
          />
        </div>
        <button onClick={send} disabled={!input.trim()||status!=="connected"}
          style={{width:46,height:46,borderRadius:"50%",background:input.trim()&&status==="connected"?"#ff6600":"#1a1a1a",border:"none",cursor:input.trim()&&status==="connected"?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:18,boxShadow:input.trim()&&status==="connected"?"0 0 20px #ff660050":"none",transition:"all 0.15s",color:input.trim()&&status==="connected"?"#000":"#333"}}>
          ↑
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [token,setToken]  =useState(()=>store.get("delivr_token")||null);
  ...
  // Fetch community patterns (on mount + every 60 s)
  useEffect(()=>{
    async function fetchPatterns(){
      try{
        const res=await fetch(`${API_BASE}/waits/patterns`);
        if(res.ok)setCommunityPatterns(await res.json());
      }catch(e){}
    }
    fetchPatterns();
  ...
  function handleLogin(userData,tokenValue){
    setUser(userData);setToken(tokenValue);
    store.set("delivr_user",userData);store.set("delivr_token",tokenValue);
  }

  function handleLogout(){
    setUser(null);setToken(null);
    store.del("delivr_user");store.del("delivr_token");
    setScreen("waits");
  }
  ...
  async function handlePickedUp(){
    ...
    const ts=new Date();
    const entry={id:Date.now().toString(),restaurantId:activeWait.restaurantId,waitMins,ts:ts.toISOString(),hour:ts.getHours(),dow:ts.getDay(),period:timePeriod(ts.getHours())};
    ...
    // Sync to server — non-blocking, patterns get pushed back via polling
    if(token){
      try{
        const res=await fetch(`${API_BASE}/waits/log`,{
          method:"POST",
          headers:{"Content-Type":"application/json","Authorization":"Bearer "+token},
          body:JSON.stringify(entry),
        });
        if(res.ok){
          // Optimistically refresh patterns right away
          const r2=await fetch(`${API_BASE}/waits/patterns`);
          if(r2.ok)setCommunityPatterns(await r2.json());
        }
      }catch(e){/* offline — will catch up on next poll */}
    }
  }
